...
try {
    ...
    # 2026-07-27以降に集中しておる型。記録簿 #19/#20/#21/#22/#23/#24/#25 が該当）。
    ...
    #
    # 【殿裁定2026-08-01＝記録に絞る】御言葉＝「キリル文字が混入してもopus4.8のように
    # 作業に支障がでなければ問題ない。記録だけ続けてほしい」。
    # ゆえに本検知では離脱プロトコル(§5)を促さぬ。促すのは記録と、意味が通らぬ場合の言い直しのみ。
    # 対比＝#1(Opus 4.8、course混入が頻発し殿指示で手動再起動)は「作業に支障が出た」型にて、
    # そちらは従来どおり§5の対象。すなわち分かれ目は「語が1つ紛れた」か「作業が滞った」か。
    if ($joined -cmatch '[Ѐ-ԯ]') {
        ...
        $reason = '【キリル文字混入検知(Stop hook機械検知)】このターンの出力にキリル文字を検知した: ' `
            ...
            + '(1) docs-notes/output-corruption-log.md へ1行追記せよ' `
            + '(型C／直前に殿の差し込み依頼があったか／同一系列の再発か否かを明記)。これが本検知の眼目にござる ' `
            + '(2) 意味が通らぬ箇所なら言い直せ。通じておるなら言い直さずともよい ' `
            + '(3) 離脱プロトコルは不要(殿裁定2026-08-01＝記録に絞る)。' `
            + 'ただし出力が頻発する・ツール呼び出しが崩れる等、作業に支障が出ておるなら §5 に従え。'
        $out = @{ decision = 'block'; reason = $reason } | ConvertTo-Json -Compress
        Write-Output $out
    }
    exit 0
