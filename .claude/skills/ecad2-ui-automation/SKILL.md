...
- 【MUST・2026-08-14追加】UAC で昇格したプロセス（インストーラー・アンインストーラー等）には、
  我らの操作は一切届かぬ。触れてはならぬ。
  - 機序＝UIPI（ユーザーインターフェース特権の分離）。昇格しておらぬプロセスから昇格プロセスへは
    入力を送れず、フォーカスも奪えぬ。我らのセッションは昇格しておらぬ
  - 実測で確かめた不成立が三つ——`SetForegroundWindow` が効かぬ／`PrintWindow` が白紙になる
    （`CopyFromScreen` で代替した）／合成クリックが届かぬ
  - 【最も危ういのは三つ目にござる】クリックは「効かぬ」のではなく、前面に在る別のウィンドウへ落ちる。
    2026-08-14、忍者がインストーラーを操作したつもりで、殿のVSCodeへクリック1回と
    スクリーンショット撮影を行う事故が起きた。撮影内容は当該セッションのターミナルログであった
  - How to apply＝昇格プロセスの操作は殿のお手にお願いする。我らは前後の観測に徹する。
    観測（レジストリ・ファイル・プロセスの読み取り）は昇格の壁に阻まれぬゆえ、そちらは従来どおり
  - 【着手前に退避せよ】昇格プロセスが絡む検証では、殿の環境が変わり得る。
    レジストリは `reg export`、ファイルはパス＋MD5の一覧を、必ず着手前に採っておく。
    2026-08-14はこれが在ったゆえ、事故の後も戻せる備えが保たれた
  - 【この条は忍者の落度から出たものではない】家老が采配文にフォーカスの扱いを一言も書かなかった
    ことが根にござる。忍者がいかに注意しても、あの操作は成立し得なかった——
    すなわち注意で防ぐ類ではなく、そもそも触れぬと決めるほかない
...
- **【MUST】入口が開いたことは、通れたことではござらぬ**——**本文は6.8節に在る**（2026-07-31、
  ...
  **他の【MUST】が「操作の作法」であるのに対し、これは「判定の作法」にござる。**
  - **【一段手前の罠も6.8節に在る】**（2026-08-16追加、忍者）——
    **「操作が通っておらぬときも、結果を読む器は値を返す」。**
    **本項が「開いたことを成功の証にするな」と説くのに対し、あちらは
    「0件・空という結果を、操作が通った前提で読むな」にござる。**
    **同じく手順を組む段では引く契機が無く、しかも失敗が朗報の顔をして来るゆえ、
    こちらより気づきにくい。** **併せて目を通されたい。**
...
#### 【別の道筋】検証の終わり方が「クリーンな状態」だと、保存操作を挟まずとも同じ結果が起きる（2026-08-06、`PR-17`(1)(2)実測、忍者）
...
#### 【上の問いに実測の答えが出た】「体で覚える」に賭けて、同じ日に二度失敗した（2026-08-16、忍者）

**上段は「手順書へ明記するか、体で覚えるか」を次の者へ委ねてござる。忍者はその判断を下さぬまま
実機へ入り、同じ日に二度とも退避を怠った。** 以下はその実測にござる。

| | 一度目（T-151 ラベル修正の実機確認） | 二度目（T-152 検体作り） |
|---|---|---|
| `Stop-Ecad2App` の帰趨 | **強制終了に落ちた**（未保存確認でブロック） | **正規終了に成功した** |
| 終了時保存 | 走らず | **走った** |
| `main-layout.xml` | **不変**（MD5・サイズ・時刻とも） | **上書き**（`LastActivationTimeStamp` 三箇所。構造は不変） |

**【最も重い一点】一度目の「実害なし」は、作法ではなく偶然が生んだものにござる。**

**一度目の後、忍者は「実害は無かった」と報じて締めた。その締め方が、二度目を招いた**
——**現に同じ怠りを繰り返しておる。**
**怠りが実害に至らなんだ時、それを「己の測り方は足りておった」と読んではならぬ。**
**読むべきは「今回はたまたま助かった」であり、次に同じ場面が来れば同じ穴が開いておる。**
（**この見立ては家老が本日足したもの**。忍者の当初の括りは
「`Stop-Ecad2App` がどちらに転ぶか事前に判じられぬ」までであった——
**それは上段 `:370`〜`:406` に既に書かれており、新しくない。**
**新しかったのは「一度目の無事をどう読むか」の方**にござった）

**How to apply＝手順書へ明記する側へ倒すこと。** 忍者は本日この判断を下し、
`docs/ecad2-t152-prep-ninja.md` の「明日の手順」の**一番目**へ
「`main-layout.xml` を先に退避する」と書き入れた。**体で覚える側は、二度の実測で否定されてござる。**

**併せて＝怠りを数えるときは回数を隠さぬこと。** 「二度とも怠った」と数えて初めて、
**偶然と作法の区別がつく。**「一度目は無事であった」と書けば、一度目は成功の側へ数えられてしまう。
...
##### 【例外・ただし履歴が空から始めた場合に限る】`False→True` の変化は使える（2026-08-16、T-152実機、忍者）

**上の条は「`True` であること」だけでは判じられぬと説く——別の操作で積まれた履歴が残っておれば
常に `True` ゆえ。**
**されど「履歴が空（`IsEnabled=False`）の状態から始めた」場合に限り、
`False→True` という変化そのものが「今の操作が履歴を積んだ」証になり申す。**

**実測（T-152 観点8＝マーカーのチェックボックスが Undo 対象か）**——

| 段 | `ToggleState` | `UndoButton.IsEnabled` |
|---|---|---|
| パーツエディタを**新規作成で開いた直後** | `Off` | **`False`**（履歴が空） |
| チェックを `Toggle()` | `On` | **`True`** |
| Undo | **`Off`（戻った）** | — |

**値が戻ったことだけを見れば「戻った」で終わるが、`False→True` は別軸で
「記録が積まれた」ことを示してござる。**

**【併せて・UIA 迂回の判定にも使える】** `TogglePattern.Toggle()` が Click ハンドラを迂回しておれば
**履歴は伸びず `False` のまま**にござろう。**ゆえにこの変化は、迂回が起きておらぬことの証にもなる。**

**【射程・忍者が自ら区切った】本例外が成り立つのは「履歴が空から始めた」場合のみ。**
**T-152 で成り立ったのは、パーツエディタを『新規作成』から開いたゆえ**にて、
**忍者はその条件を意識して選んだのではなく、`図形` 配下を汚さぬために新規を選んだ結果であった。**
**既存のパーツを開いて何か操作した後であれば、上の条のとおり `IsEnabled` は当てにならぬ。**
**すなわち本例外は上の条を覆すものではなく、上の条が指摘した限界の入口を一つ示すだけにござる。**
**測る前に `IsEnabled` が `False` であることを確かめ、`False` でなければ上の条（回数を数える）へ戻ること。**
...
#### 【射程の拡張】同じ手で「テキスト」も採れる——回路番号・行番号・表題（2026-08-16、T-151実機確認、忍者）

**上節は座標の話にござるが、`Tj` 演算子を拾えば描かれた文字そのものが採れる。**
展開までの手順は上節と同一（`FlateDecode` を `DeflateStream` で展開・zlibヘッダ2バイトを飛ばす）にて、
**そこから `Tj` のテキストを拾うだけ**にござる。

**値打ち**＝**キャンバスに描かれた数字を機械的に採る、今のところ唯一の手。**
`CanvasArea` は単一ビジュアルとしてしかUIAツリーに現れぬゆえ（4節）、
**内部の文字はUIAでは取れ申さぬ。** 拡大画像の目視に頼らず二軸目を立てられる。

**実例＝T-151**。回路番号の並びを
`1 2 3 4 5 4 6 7 8 9 7 8 10 11 12 13 14 12 13 15 16 17 18` として採り、
**15・16・17・18 が各1回ずつ**であることを機械的に確定した（拡大画像での実見と一致）。
**行番号（1〜24）と表題（`R200`／`S200`／`AC200V`）も同じく採れる。**
証跡＝`sample/t151-ninja-circuitnum.pdf`。

**【限界その一・日本語は `Tj` に現れぬ】** 機器名等の日本語文字列は拾え申さぬ。
**別の演算子かグリフ参照と見られるが、機序は確かめておらぬ。**
**採れるのは英数字と記号に限る**と踏まえて観点を組むこと——
**「日本語ラベルの有無をこの手で測る」は成り立ち申さぬ。**

**【限界その二・`Td` は相対座標ゆえ絶対位置が取れぬ】**
座標で「どの文字がどこに在るか」を同定する道は塞がっており申す。
**T-151では描画順（行番号24件の後が回路番号）で切り分けた。**
すなわち**何が何番目に描かれるかを別途知っておらねば、拾った文字列の同定ができぬ**——
**この点、座標が直に読める図形（上節）とは性質が違う。**

#### PDFを得るまでの操作手順（2026-08-16、T-151実測、忍者）

**上二節はPDFが手元に在る前提にござる。得るまでの道はこう。**

1. `PDF出力 (Ctrl+P)`（ツールバーボタン。Nameは4節既載）
2. プレビュー窓 `PDF プレビュー` が開く
3. **その窓の中の** `PDF出力` ボタンを押す
4. `名前を付けて保存` ダイアログが開く
5. **フルパス**を `Send-Ecad2Keys -Force` で入力 → `Enter`

**フルパスで入れる理由**＝直前の別ダイアログのカレントフォルダを引き継ぐ罠（6.6節）。
**`-Force` が要る理由**＝21文字超の文字列ガードの明示回避（同節）。

**T-151では一度で通り申した**——**同日朝に難儀したファイルダイアログ（検体を「開く」側）とは別物**にござる。
開く側は引数付き起動で丸ごと迂回できる（6.6節末）が、**保存する側にその逃げ道は無い。**

**【未記録】プレビュー窓の中のボタンをどう掴んだかは、T-151の記録に残っておらぬ。**
**プレビューが別ウィンドウである点は6.3節に既載**（PrintWindow撮影の色が信頼できぬ件・
`Ecad2Native.PrintWindow` を直に呼ぶ運用）ゆえ、
**メインウィンドウの `FindAll` からは辿れぬ前提で組むこと**（別ウィンドウの掴み方は6.1節）。
...
- **「名前を付けて保存」ダイアログの既定フォルダは、同一セッション内の直前の別ダイアログ操作
  ...
  フォルダの引き継ぎ問題を回避できる。
- **【PDF出力の保存ダイアログへ至る道は6.4節にまとめてござる】**（2026-08-16、T-151、忍者）——
  `Ctrl+P` → プレビュー窓 → その中の `PDF出力` ボタン → 本ダイアログ、という二段の窓を経る。
  **上記の「フルパスで入力」「`-Force` で21文字ガードを外す」はそのまま効く。**

#### 配置バー（`ElementPlacementBar`）は常時表示ではない。出すには三手が要る（2026-08-16、T-153検証、忍者）

**配置後入力の浮動バーに在る要素は、バーを出さねば UIA で一切取れぬ。**
`MainWindow.xaml:1923` の `ElementPlacementBar` が
`Visibility="{Binding IsPlacementBarVisible, …}"` に束ねられておるゆえ。

**バーの中に在るもの**＝機器名／コメントの入力欄（`PlacementDeviceNameBox`）・種別コンボ・
`OK`／`キャンセル`・左端の小アイコン・末尾の `拡張表示`（T-153 で一部が非表示化）。

**出す手順は三手**——**二手目で止まると「配置位置を指定してください」で待つ。**

```powershell
Invoke-Ecad2CanvasClick -RelativeX (304+34*列) -RelativeY (255+34*(表示行-1))  # 1. セルを選ぶ
# 2. a接点配置(F5) 等を InvokePattern で起動（例外が飛んでも効いておることがある）
Invoke-Ecad2CanvasClick -RelativeX (304+34*列) -RelativeY (255+34*(表示行-1))  # 3. もう一度クリック＝配置位置の指定
```

**バーが出たかの健全性確認＝ボタン総数**（忍者の実測：バー非表示24／表示29。
T-153 適用後は27）。**個別要素の在/不在で判ずる前に、総数で「バーが出ておるか」を先に見ること。**

**【この節が生まれた経緯——半日を空費した】** 忍者は T-153 の弁別のため
「`拡張表示` が UIA で取れるか」を測ったが、**バーを出さぬまま測って「取れぬ」を得た。**
**危うく「T-153 が三つとも消した」と誤報するところであった。**
**露見の契機はボタン総数の食い違い（20 対 29）**——**数を並べて記録しておいたゆえ気づけた。**

**さらにその前段で「インストール版では三つとも取れる」と報じており、それも不正確であった**
——**あれは殿のプロセスで偶々バーが出ておった状態**にて、条件を明記せぬまま述べたもの。
**同じ測定を二度、条件を落として行っておる。**

**【印を渡された時の作法】**（同日、侍との遣り取りから）——
**侍が「`拡張表示` が取れぬことが T-153 の印」と示し、忍者がその印を条件抜きで確かめた。**
**双方が「その印が成り立つ条件」を落としておった。**
**次に侍は「起動もバー表示も要らぬ印」（dll のメタデータ走査）を選び直しておる。**
**印を受け取ったら、印そのものだけでなく「どの条件で成り立つか」を併せて問うこと。**

#### 検体を「開く」だけなら、ダイアログを使わず引数付き起動が確実（2026-08-16、T-151実測、忍者）

**`Ecad2.App.exe <パス>` で開けば、ファイルダイアログの操作が丸ごと要らぬ。**
**T-123で `.gcad` 関連付けのために加えられた起動引数が、そのまま検証の道具になる。**

```powershell
Start-Process -FilePath 'C:\ECAD2\src\Ecad2.App\bin\Debug\net10.0-windows\Ecad2.App.exe' `
              -ArgumentList 'C:\ECAD2\sample\big_sample.gcad'
Start-Sleep -Milliseconds 5000
Move-Ecad2WindowToScreen -Screen Secondary
```

**なぜ避けるか**——同日、`Send-Ecad2Keys` によるパス入力が二度続けて黙って失敗した。
**一度目は対象ダイアログが前景であることを確かめたうえで届かず、二度目は前のダイアログが
閉じておらず多重化した**（上項）。**いずれも例外は出ず、図面が開かぬまま検証が進んだ**
——**その先で何が起きるかは 6.8節「操作が通っておらぬときも、結果を読む器は値を返す」にござる。**

**併せて**——`Start-Ecad2App` は `dotnet run` ゆえビルドが走る。
**侍が実装を進めておる間は、exe を直接起動する方が `bin` を汚さずに済む**（本項の形なら読むだけ）。

**限界**＝`Start-Ecad2App` が備える待機・セカンダリ移動・標準出力のリダイレクトは付いてこぬ。
**起動待ちと `Move-Ecad2WindowToScreen` は自分で書くこと。**

**【併せて実測・`Start-Ecad2App` の10秒待機は初回ビルドで足りぬ】**（同日）——
**`dotnet run` がビルドから始まる場合、`Ecad2.App did not start within 10 seconds` で例外になる。**
**されどプロセスはその後に起ち上がっており、起動自体は成功しておる**（実測＝13秒ほどで確立）。
**例外を見て「起動せず」と判ぜず、`Get-Ecad2Process` で確かめ直すこと。**
**その場合セカンダリへの自動移動も行われておらぬゆえ、`Move-Ecad2WindowToScreen` を自分で呼ぶ。**
...
- **【MUST】操作が通っておらぬときも、結果を読む器は値を返す**（2026-08-16、T-151の実測および
  `DRC-XREF-001` の調査、忍者が同日三度踏んだ）。
  **上項が「途中経過のUIを成功の証にするな」と説くのに対し、本項は一段手前にござる**——
  **操作そのものが通っておらぬのに、結果側の指標が「0件」「空」という、いかにも読める値を返す。**

  **同日の三件**——

  | 測ろうとしたもの | 通らなんだ操作 | 返ってきた値 | 実相 | 危うく何と報ずるところであったか |
  |---|---|---|---|---|
  | DRC件数 | `Send-Ecad2Keys` がダイアログへ届かず図面が開かなんだ | DRC 0件 | 図面が無い | 「対照で症状が全て消えた」 |
  | 同上（2度目） | ダイアログが閉じず多重化 | 同上 | 同上 | 同上 |
  | DRC件数 | メニュー項目を掴み損ねDRCが走らなんだ | DRC 0件 | 診断が未実行 | 「外部接点を置いたら全部消えた」 |

  **「0件」は二通りに読める**——**問題が無いのか、測っておらぬのか。**
  **器は「測って0件」と「そもそも測っておらぬ」を区別せぬ。**
  **`OutputGrid` の `ScrollPattern` で全件性を確かめても無駄にござる**——
  **空の器はいつでも「全件表示・スクロール不要」と正しく答えるゆえ。**

  **How to apply**
  1. **否定的な結果（0件・変化なし・空）を得たら、まず「操作は通ったか」を別の指標で確かめる。**
     **DRCなら図面が読まれておるか**——`保存`ボタンの `IsEnabled`／シートリストの中身／
     スクリーンショットの実見。**三件とも、これで割れた**
  2. **着手前に数値の予測を書いておく**（`ninja.md:216` の症状の数値翻訳が、思わぬ形で効く）。
     **三件目を捕らえたのは「外部接点aは置いておらぬゆえ最低1件は残る筈」という予測が在ったゆえ。**
     **予測が無ければ、0件は良い知らせに見える**
  3. **例外が出る形に組む。** **三件目は `$drc` が null で例外が出たゆえ即座に判った**——
     **無言で空を返す形より、例外で止まる形の方が安全にござる。**
     **UIA要素の取得は `throw` せず null を返すゆえ、受けた側で明示的に確かめること**

  **【なぜ「気をつけよ」で済まぬか】三件とも、都合の良い方向へ外れておった。**
  **いずれも「症状が消えた」＝望ましい結果に見える形で失敗しており、疑う動機が働きにくい。**
  **失敗が不都合な形で現れるなら誰でも気づく。危ういのは、失敗が朗報の顔をして来る場合にござる。**

  **関連**＝`ninja.md:424`（**疑うべきは観測対象だけでなく、観測手段そのものでもある**）。
  **あちらは「フィルタの綴りが対象を覆わぬ」型、こちらは「操作が通らず結果が空になる」型**にて、
  **いずれも測り方が答えを先に決めておる。**
  **併せて 6.6節「ファイルダイアログを使わず引数付き起動で開く」**——
  **本項の一件目・二件目は、そちらの手を採れば元より起こらなんだ。**
- 【プロセスの不在確認は、全プロセスの増減で測れ】期待する名だけを見れば、別のアプリの起動が
  ...
  また6.3節「拡大して判じたものは実寸の見え方ではない」（2026-08-16時点で `:971`。
  当初の `:947` からずれておる）とも同日・同じ筋——
