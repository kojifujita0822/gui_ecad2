...
#### 【併せて・別軸】拡大して判じたものは、実寸の見え方ではない（2026-08-14、忍者・侍・家老が同じ方へ倒れ、殿の実見が覆した）

上の三小節は「目視か画素か」という判定手段の話にござる。本節は別の軸——観測のために施した加工が、
対象の性質そのものを変えてしまう形にござる。

実例＝新アイコンの16px判読性。三者が別々の役から一致した。

| 誰 | 手段 | 判 |
|---|---|---|
| 侍 | 画面上の拡大図を作って見た | 案Cは16pxで三案中最も線が細い |
| 忍者 | 実機を拡大撮影した | 「2」の字形がやや潰れ判じにくい |
| 家老 | 忍者の拡大画像を見た | この寸法では「2」がほぼ判じられぬ |

殿が実寸でご覧になり——「16pxでも判別できた、問題ない」。三者の一致が覆り申した。

機序の見立て＝拡大はアンチエイリアスの中間色や字形の潰れを引き伸ばして目立たせるが、実寸では
その粗が視野の中で埋もれる。三者とも拡大という同じ手段を使うたゆえ、判断も揃うた。

How to apply＝「読めるか」「判別できるか」を問う検証では、拡大画像だけで確定させぬ。
実寸（またはそれに準ずる縮小）を併せて見る。最終の判は殿の実見を仰ぐのが筋にござる。
報告する側は「拡大して判じた」ことを明記されたい——忍者は本件でそれを正しく区切っており、
その断りが在ったゆえ家老は所見を確定として扱わず、殿へ実見を仰ぐ形を採れ申した。

【射程を区切る】本件は `n=1` にござる。しかも「三者が揃うた理由は拡大という手段であった」という
因果そのものは、誰も測っておらぬ——三人が同じ絵を見て同じ印象を持っただけかもしれ申さぬ。
上の機序は家老の見立てであって、忍者・侍が述べたものではござらぬ。
ゆえに本節が約束するのは「拡大だけで確定させるな」という手順までにて、
「拡大は必ず判断を厳しい側へ倒す」とまでは申しておらぬ。

【上の「比の限界」との違い】`:926` の比の限界は、比較の軸（2色の関係）が何を見落とすかの話にござる。
本節は、観測のために施した加工（拡大）が対象の性質を歪める話にて、別の罠にござる。
...
- 【プロセスの不在確認は、全プロセスの増減で測れ】期待する名だけを見れば、別のアプリの起動が
  「起動せず」に見える（2026-08-14、T-123インストーラー実機確認、忍者）。

  実例＝`.gcad` 関連付け経由の起動確認で `tasklist | grep -i ecad2` のみを見ておった。
  されど `GuiEcad.App.exe` には「ecad2」という並びの文字列が含まれぬ（G-u-i-E-c-a-d）。
  このフィルタでは構造上、検出できぬ。

  実際には `UserChoiceLatest` が `GuiEcad.Document` を握っており、ダブルクリックで GuiEcad が
  起動しておった。忍者は Ecad2 側のプロセスのみを見て「いずれも新規プロセスが起動せず」と誤って報じた。
  露見したのは侍の問い——「シェルが GuiEcad と解決する以上、GuiEcad が起動する筈。噛み合わぬ」——
  および殿が実機でダブルクリックし GuiEcad が開いたことによる。

  How to apply＝起動せぬことを確かめる際は、期待するプロセス名の不在ではなく、
  操作前後の全プロセス一覧を採って増減の差分で測る。
  名前で絞り込むなら、競合しうるアプリの名前も併せて挙げる。

  とりわけ関連付け・シェル解決が絡む検証では、どのアプリが応じるかが分からぬ。
  すなわち絞り込みそのものが「ecad2が応じるはず」という前提を含んでしまう。
  測る前に答えを決めておる形にござる。

  【この条が注意で防げぬ理由】忍者の言＝「これは構造的な欠陥にござる」。
  フィルタの綴りが競合アプリの綴りを覆わぬことは、書いた本人には見えぬ。
  気をつけて見直しても、`grep -i ecad2` は正しく見える——探す対象が ecad2 なのだから。
  ゆえに要るのは注意ではなく、絞り込まぬという手順にござる。

  関連＝上の「入口が開いたことは通れたことではござらぬ」と同根にて、いずれも結果側の指標を
  正しく取れという話にござる。あちらは途中経過のUIを成功の証にした誤り、こちらは期待するものだけを
  見て他の可能性を見落とした誤り（忍者の整理）。
  また `:947` の「拡大して判じたものは実寸の見え方ではない」とも同日・同じ筋——
  いずれも測り方が対象を歪める話にて、本日忍者が二件を落としてござる。
