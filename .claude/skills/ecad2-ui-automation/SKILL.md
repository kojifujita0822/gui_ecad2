...
**理論値と 0.05mm 以内で一致**し（T-133では X1＝20.00→300.13mm、X2＝173.0→453.13mm を実測）、
**要素の同定にも使える。**

**併せて採れるもの**＝`MediaBox`（用紙サイズ。A4縦なら `[0 0 595.276 841.89]`）。
**MediaBox を越える座標がそのまま書かれておるか**を見れば、**クリップの有無が判る**
（T-133ではこれで「切られておらぬ」を確定した）。
...
**`CanvasArea` の Bounds は 888px 幅だが、セル選択が実際に効いたのは rel x≒900（列17）まで。**
**それより右は叩いても選択セルが変わらぬ。**

**Bounds を信じて座標を決めると空振りする**——**「叩いたが何も起きぬ」を「操作が効かぬバグ」と
誤診断しかねぬ。** **範囲の端を使うときは、まず効く範囲を二分探索で押さえること。**
...
**パーツエディタの線ツールでも、136px／40刻みでは引けず**（下書きが始点＝終点のまま縮退して
捨てられる）、**340px／68刻みで成立した。**
