...
Stop-Ecad2App
# ★★ `Stop-Ecad2App` は `Stop-Process -Force` ＝プロセスの強制終了であり、
#    `Window_Closing` を経由しない（T-128実測2026-07-27、忍者が事故として発見）。
#    ゆえに**終了時に走るはずの保存処理が一切走らぬ**——実例＝`SaveDockingLayoutAsDefault()`
#    （`MainWindow.xaml.cs:1443-1451`）が呼ばれず、**検証中の一時的なパネル配置が
#    `main-layout.xml` にそのまま残る事故**が起きた（左パネル532pxのまま殿の環境に残留、
#    忍者が直後に気づき復旧）。
#    → **終了時の挙動・永続化に関わる検証では `WindowPattern.Close()` で正規終了させること**
#       （`Window_Closing` を経由するため、実際のユーザー操作と同じ経路になる）。
#    → 併せて、**「終了時に何かが保存される」機能そのものを検証する場合、`Stop-Ecad2App` では
#       検証にならぬ**。未保存確認ダイアログ等、他の終了時処理も同じく飛ばされる点に注意。
...
- **AvalonDockのタブ切替式ツールバーは、非選択タブの内容がUIA要素ツリーから完全に消失する**
  ...
  確認すること**。前掲のDataGrid仮想化（288-293行）と同型だが、こちらはスクロールでなく
  タブ選択が条件。実装バグと即断する前に、タブを切り替えてから再探索する。
- **メニューのPopup表示中は座標クリック（`Invoke-Ecad2ScreenClick`）が不発になることがある**
  （2026-07-25、T-068増分2検証で3回再現）。Popupが閉じもせず、クリック自体も効かないという
  二重の空振りになる。`InvokePattern`経由へ切り替えて解消した。**直前の604-608行の教訓
  （標準MessageBoxのボタンは逆にInvokePattern非対応ゆえ物理クリックが要る）とちょうど裏返しの
  関係にあるため、混同しないこと**——標準MessageBoxのボタン＝物理クリック、メニューのPopup
  項目＝InvokePattern、と対象ごとに使い分ける。どちらも「片方の手法だけで粘ると空振りが続く」
  という点は共通ゆえ、同じ操作を3回試して症状が変わらなければ、実装バグを疑う前に**もう一方の
  操作手法へ切り替えてみる**のを定石とする。
...
- **パーツエディタ等のダイアログで、ウィンドウ相対Y=150程度まではプロパティ欄・ツールバー領域であり
  キャンバスに届かない**（2026-07-25、T-068増分3-c検証）。基準枠が小さい・上端付近を狙う操作では、
  **事前にスクリーンショットで基準枠の実位置を確認する一手間**を惜しまぬこと。座標の当てずっぽうは
  「操作が効かない＝実装バグ」という誤診断を招く（実際、本日の検証でドラッグ開始座標がツールバーに
  重なり図形描画自体が失敗した結果、「編集保存で形状が消える」という重大バグに見える事象が発生した）。
- **ダイアログを閉じた後、そのウィンドウハンドルへの`GetWindowRect`は(0,0,0,0)を返す**（2026-07-25、
  T-068増分3-c検証）。**前回の位置を使い回さず、ダイアログを開くたびに必ず取り直すこと**——1回省略
  したために余計な切り分けを招いた実例あり。既存教訓「生成直後ウィンドウの`GetWindowRect`異常値」
  （`memory: ecad2_ui_automation_pitfalls_t099`）と対をなす、**閉じた後**の罠。
