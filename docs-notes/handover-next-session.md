...
- **増分B＝修正往復1周目**：侍実装`e45c2d3`（ValidatePlacement下限0・案A対応表・RED2種・197件
  ...
  →**忍者実機検証中**（観点注記つき。往復1周ゆえStryker棚卸し対象[2周以上]には非該当=家老判断）
...
3. **忍者の増分B実機検証結果を受けたら**：OK→増分Bクローズ→**侍**へ増分C（View層=
   TryPlaceElementの検証関数追随+UXフィードバック+RowAtDip死にコード除去）采配。NG→範囲内欠陥
   なら往復2周目（2周までは家老裁量、3周超で殿上申=中断基準）
