...
- **増分D（ドラッグ外枠共通化）＝フェーズ1（侍の具体案提示待ち）**（采配2026-07-09）：計画書
  明記の手順=侍がForceCancel共通化方式（delegate案vs個別維持）とConfirm/Cancel骨格PoC対象1種の
  具体案を提示→家老判断→実装采配。UpdateDrag*実クランプは型ごと維持（共通化しない）。
  UI挙動不変の純リファクタゆえ殿確認不要（計画書裁可済み範囲）
