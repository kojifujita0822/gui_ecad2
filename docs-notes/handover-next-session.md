...
3. **増分B修正往復1周目の続き**：隠密のテスト設計書→侍の修正+テスト実装（RED先行証明必須）→
   隠密再レビュー→忍者実機（配置操作全般の回帰＋機器表「種別」列の英語名表示。**観点注記**：
   境界外セルへの配置試行の無反応UIは増分C対処予定の既知事項ゆえバグ扱いしない。代わりに
   データ非設置＝機器表・DRCに出ないこと[P-024再発なし]を確認）——往復2周までは家老裁量、
   3周超で殿上申（中断基準）
