...
- **侍が「増分A補遺（テスト補強）」→「増分B（P-025 VM層）」の順で実施中**（采配2026-07-09、
  別コミット・ブランチ維持）。補遺完了後は隠密レビューのみ（ロジック変更なしのため実機不要=
  家老判断）、増分Bは隠密レビュー→忍者実機の既定順
...
2. **侍**の「増分A補遺」完了報告を受けたら→**隠密**へレビュー采配（所見1・2の盲点が実際に
   塞がれたか＋RED証明の整合。実機不要）
