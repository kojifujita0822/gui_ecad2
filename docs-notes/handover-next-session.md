...
- **増分D（ドラッグ外枠共通化）＝実装中**（フェーズ1完了→家老判断2026-07-09：
  ...
  **mainマージは忍者最終回帰OKが揃うまで保留**（殿裁定条件の未達につき）。
  ...
  追記済み（`bd0d033`）。
  →**補遺2実装完了`dc81b74`**（notify発火検証4種×3パターン=12箇所・全20値[Theory]・RED証明
  2系統実測・228件全合格=Core14+App214。技術的判断=MapToDeviceClassはリフレクション経由で
  private直接検証［実経路はPartRole制約で7値のみ到達可、13値は経路上テスト不可能のため］）
  →**隠密差分レビュークリーン＝補遺2クローズ確定（2026-07-09、
  `docs/ecad2-t045-addendum2-review-onmitsu.md`）**。DoD(4)所見=侍の「実経路7値のみ到達可」は
  不正確（隠密実査：直接Sheet.Elements操作で16値到達可、原理不可はCreatesComponent=falseの
  3P系+Motorの4値のみ）だが「実経路だけで全20値カバー不可」の結論は正しくリフレクション採用は
  妥当・実行時NREで即発覚する壊れ方＝**家老裁定：記録のみ**（InternalsVisibleTo化はsrc変更を
  要し現時点の価値薄、将来の任意改善）。
  **→T-045の静的作業は全完了。残るは殿帰還（PC再起動）後の忍者最終実機のみ**
...
2. **忍者へ増分D最終実機の再開を采配**（環境復旧=PC再起動後）。観点＝4種（VerticalConnector/
   WireBreak/FreeLine/ConnectionDot）のドラッグ移動・端点リサイズ・Escキャンセル・外的要因
   キャンセル（シート切替等ForceCancel→位置復元）一巡＋軽い全体スモーク（起動・シート切替・
   通常配置1件）。純リファクタゆえ挙動差ゼロが期待値
3. **忍者全OK→T-045全増分クローズ→mainマージ**（殿裁定2026-07-09：条件=全増分パイプライン
   全段通過・全テスト合格実測[現在228件]・忍者最終回帰OK。満たせば家老裁量で
   `git merge --no-ff feature/t045-app-layer-refactor`→main push→殿へ事後報告。
   マージ前に`git log origin/main..HEAD`で混入チェック）
4. マージ後：**侍へP-040小タスク采配**（承認済み2026-07-09、Dispatcher直接依存の再発防止
   アーキテクチャテスト1本、main上で）。T-045クローズをtodo.mdへ反映
5. pending（殿判断待ち、急がない）：P-036〜P-038・P-031/P-032/P-035・P-041（新規、種別切替
   直後の高速クリックで旧種別配置=両論併記）
