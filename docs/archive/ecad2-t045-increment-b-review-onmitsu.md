...
T-043隠密レビュー（`docs/archive/ecad2-t043-review-onmitsu-2.md`所見1、CONFIRMED）で既に問題視・修正
...
`docs/archive/ecad2-t045-implementation-plan-samurai.md`増分B節と実装を突き合わせ、`ValidatePlacement`
...
ファイルコピー、`docs/archive/ecad2-t035-review-onmitsu.md`で実機検証込みの正当操作として設計対象）が
生む重複を解消するための既存機構。
...
> T-043往復2周目(隠密レビューCONFIRMED、`docs/archive/ecad2-t043-review-onmitsu-2.md`所見1): 判定は
...
`PartEntryToGlyphGeometryConverter.cs:17-23`、`docs/archive/ecad2-t043-review-onmitsu-2.md`所見1、
`docs/archive/ecad2-t035-review-onmitsu.md`）。**DoD(4)「配置の全経路（部品選択リスト・自作パーツ経由
...
バックもない（配置バーが閉じるだけで無反応に見える）。**ただし`docs/archive/ecad2-t045-implementation-plan-samurai.md:107-123`「増分C：P-025 View層」に、まさにこの追随（`TryPlaceElement`が増分Bの
