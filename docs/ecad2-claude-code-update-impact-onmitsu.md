...
## 6. 追加調査（2026-09-14、家老の単独調査。殿ご下命）

現行バージョン＝2.1.270（`claude --version`実測）。前回調査(v2.1.226)からv2.1.270までの44バージョン分、
公式CHANGELOG.md（`raw.githubusercontent.com/anthropics/claude-code/main/CHANGELOG.md`）をcurlで
直接取得し、v2.1.227〜v2.1.270の区間を全文通読した。手法は前回と同じ（一次ソース直読）。

### 結論（要点のみ）

1. 良い影響——4セッション並列運用の弱点が直接解消された（v2.1.259）。「複数セッションが互いの
   `~/.claude.json`変更を静かに上書きし合う」不具合が修正され、多数のセッションを同時に動かしても
   workspace trust・MCP/project stateが失われなくなった。ecad2の家老/侍/忍者/隠密4プロセス並列運用は
   まさにこの不具合を踏みうる構成であった。
2. 公式cross-sessionメッセージング機能が大幅強化された（v2.1.232系）。`@`メンションでの直接送信・
   同名セッションの自動リネーム・Windows対応（v2.1.239）・`notify_when_idle`のオプトイン追加
   （v2.1.236、`startup-auto.md`が既に予備経路として言及している機能そのもの）等。claude-peers
   （MCP）とは別系統だが、buildin `SendMessage`/`ListAgents`の信頼性が上がった。
3. Permission rule関連の修正が多数入った（v2.1.268・v2.1.269等）。symlink経由のdeny rule回避、
   `env -C`/`eval`を挟んだコマンドでのdeny rule抜け穴、`!`で始まるdeny/askルールが書かれた
   settings source外まで適用されてしまう不具合等が塞がれた。ecad2の`.claude/settings.json`の
   実際の記述次第で影響しうるが、実地確認は本調査の範囲外（気になる挙動が出た時に着手すればよい）。
4. 出力破損（`<invoke>`生表示等）に直結する修正の記述は、今回も確認範囲内で見当たらなかった。
   前回結論（現行の回避運用＝Grepの`content`モード回避＋`files_with_matches`/`count`＋`Read`直読の
   継続）は変わらず妥当。
5. サブエージェント関連——v2.1.232で「forkサブエージェントは既定でバックグラウンド実行」に変更
   （`subagent_type: "fork"`は会話全体とプロンプトキャッシュを引き継ぐ）。v2.1.251で並列サブ
   エージェント多数時のTUI表示遅延（進捗表示が積み重なる不具合）が改善された。

### 我らが手を打つ要のあるもの

無し。1・2は運用にとって追い風、3は現行`settings.json`の内容次第（気になる挙動が出た時に確認すれば
足りる）。累計200件上限撤廃（前回訂正済み）以降、サブエージェント委譲の上限に関する新たな変更は
確認範囲内で見当たらなかった——`同時実行20件・ネスト深さ3`が引き続き現行の仕様と見てよい。

### 落とし先（追加分）

- 本節自体＝次回バージョン確認時、前回(v2.1.226)・今回(v2.1.270)の両方を踏まえて差分を追うための
  参照先として残す。次に確認する者は`v2.1.270`以降の区間のみを読めばよい。
