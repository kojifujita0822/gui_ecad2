...
**現況**: **WPFに正式確定**（2026-07-03、殿裁定）。T-002/T-006のPoCでフォーカス保持・大量記号描画・PDF出力・タブ切替時のフォーカス対策を実機検証済み、T-005で非技術面（ライセンス・保守体制・採用実績）も確認済み。以降は `docs/todo.md` T-004（雛形/アーキ設計）へ進む。

**注記（T-004着手時に判明・訂正）**: `C:\Program Files\GuiEcad` はインストール済みバイナリのみだが、実ソースは `C:\Users\kojif\Desktop\生産物\gui_ecad\src`（GuiEcad.App/GuiEcad.Core/GuiEcad.Pdf）に存在することを殿より確認。本書「C#資産を無傷で活かせる」は当初想定通り**コードの直接参照・再利用が可能**。
