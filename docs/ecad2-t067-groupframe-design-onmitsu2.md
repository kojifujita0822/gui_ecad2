...
> 【2026-09-09追記＝T-155で案Y・P-050対処が実現した】本調査当時 T-067 は論点2の案X
> （グリッドセル単位限定、`Visual*Mm` 不使用、P-050 は構造的に非該当）で実装された。その後、
> 殿ご下命でグループ枠の mm 自由リサイズ（T-155）が入り、案Y（`Visual*Mm` 使用）へ移行した。
> 本調査が予告したとおり `RowOps.InsertRow`/`DeleteRow` への `Visual*Mm` シフト追加（P-050 対処）が
> 必須となり、同タスクで実施済み。枠は「作成直後＝グリッド式」「リサイズ後＝自由式」の二態を持つ
> 折衷形になった。詳細は `docs/todo.md` の T-155 節。
