...
### 観点2：クランプ→並べ替えの順序 — 【訂正】侍の実装・根拠説明とも正しい。拙者の反証は誤りだった

**当初、拙者は「`Math.Clamp`は単調写像・`OrderBy`は安定ソートゆえ、順序をどちらにしても最終結果は
常に同一になる」と結論したが、これは誤りだった。侍・家老の反例検証を受け、率直に訂正する。**

見落としの核心＝**複数の接続点がクランプ後に同一値へ潰れる場合**、安定ソートのタイブレークが
「どの時点のリスト順を保持するか」に依存し、クランプ前後で結果が変わりうる。拙者が机上検算で
試した反例は、いずれも「クランプ前に既に大小関係が正しい順序だったケース」に偏っており、
「クランプ前は逆順（大きい値が先）だが、クランプ後は同値に潰れる」という決定的なケースを
一度も試していなかった（PR-27型に通じる、反証の網羅性不足という自己の穴）。

侍の反例で機械的に再検証した：
```
ports = [A(boundary=5), B(boundary=3)], widthCells=2（許容[0,2]）
現行方式（クランプ→並べ替え）：クランプでA→2, B→2（同値）→安定ソートは
  クランプ後リストの順序[A,B]を保持 → 結果 [A(2), B(2)]
逆順（並べ替え→クランプ）：まず元の値で並べ替え→[B(3), A(5)]→クランプ→
  結果 [B(2), A(2)]
```
**[A,B] と [B,A] で順序が異なる——侍の反例は成立する。** Pythonでの機械的再現でも同一結果を確認した。

**訂正後の結論**：`Math.Clamp`が単調写像であることは「大小関係が逆転しない」ことしか保証せず、
「同値に潰れた場合にどちらが先に来るか」には無関係。**現行方式（クランプ→並べ替え）は、
「クランプ後の値で見た時、元のリスト順（クランプ処理は各要素の値を変えるだけでリスト順序自体は
保持する）をタイブレークとして使う」という、直感的で意味のある挙動になる。逆順（並べ替え→
クランプ）だと、「クランプ後は無意味になった、クランプ前の値による大小関係」がタイブレークとして
固定されてしまう不自然さがある。** 加えて「先頭=NetA・末尾=NetB」の規約により、この順序差は
**電気的な意味を持つ実害**（どちらの接続点がNetAになるか）であり、家老・侍の指摘どおり順序の
選択自体に実質的な正当性がある。

**評価（訂正版）**：侍の実装（クランプを先に行う）・根拠説明（逆順だと並びが崩れる）は**いずれも
正しい**。拙者の反証は「大小関係の保存」のみに着目し「同値タイの場合の相対順序」を見落とした
誤りであり、撤回する。
...
6観点いずれも機能バグは無し。**観点2は当初拙者が誤った反証を報告したが、侍・家老の反例検証を
受け訂正した——侍の実装・根拠説明とも正しい。** 観点5（コメントの部分的不正確性）のみ軽微な
記述上の指摘として残る。本方式（保存時のみクランプ、編集中は原本どおり不変）は、前回発見した
「Undo中間状態での非可逆な誤クランプ」のリスクをも構造的に消滅させており、setter方式より堅牢と
考える。忍者の実機確認へ回してよいと判断する。
