...
### 3.1 案イ（隠密推奨）：ecad2独自メニューでFloat/Dock操作を代替提供

T-110増分3のAutoHide代替UI（表示メニュー「パネルを自動的に隠す」サブメニュー、`Tag`ベースの汎用実装）と同じ設計思想で、表示メニューへ「配置ツールバーをフロート化/ドッキング」相当の項目を追加する。対象は`PlacementToolBar`のみ（`MainToolBar`はCanFloat="False"のため対象外）。

- 発動：`LayoutAnchorable.Float()`（`LayoutContent.cs`、public、T-099(c)調査4で存在・アクセシビリティとも確認済み）を呼ぶ。
- 復帰：フロート中に同項目を再度選択すると`Dock()`を呼ぶ（トグル）。ただし**T-099(c)調査5で確定した「メニューDockでのタブ自己複製バグ」（`InternalDock`のフォールバック探索にフロートウィンドウ除外フィルタが無い）が既知の未解決課題として存在する**——これは配置ツールバー領域が「単一ペイン・単一Anchorable・独立DockingManager」だった旧構成（4分割DockingManager時代）を前提にした調査であり、T-110で単一`MainDockingManager`へ統合された現在も同じ脆弱性が残るかは要再確認。もし残っているなら、メニューからのDock項目には対処案Y相当（ハードコード既定レイアウトDeserialize、`ecad2-t099-c-...-onmitsu.md`調査6）の組み込みが必要になる可能性がある。
- ドッキング復帰は上記メニューに加え、既存のタブドラッグ（§2.3で機能確認済み、アプリ内→再ドッキング領域への手動ドラッグ）でも可能なため、メニューのDock項目自体を省略し「Float発動のみ」に絞る簡略案も取りうる（フロート解除はタブドラッグに任せる）。
- **得**：技術的に確実（`DockingManager.Resources`の伝播問題を回避、§2.2の不確実性に依存しない）。実装規模小。AutoHide代替UIとの一貫性が高く、利用者が「表示メニューを見ればパネル操作は揃っている」と学習しやすい。
- **失**：フロートウィンドウ自体の見た目（帯）は変わらない（VS2013既定のまま、Collapseされた状態が続く可能性がある）。「タイトルバーが表示される」という殿裁定の字面どおりの実現ではなく、機能面（Dock/AutoHideへのアクセス手段確保）での代替になる。
...
## 4. 新規の気づき：AutoHide操作の入口喪失（要検討事項に無かった論点、殿確認事項T121裁2）
...
1. **フロートウィンドウ内の`LayoutAnchorableControl`が`MainDockingManager.Resources`の暗黙的スタイルを引き続き受け取るか**（§2.2）：2件の独立調査で「届かない」可能性が高いと判断したが、`LayoutFloatingWindowControl`基底クラス側の未確認経路が皆無とは言い切れない。案ロ・案ハを検討する場合はこの点の追加確認（または実装後の実機確認での裏取り）が前提になる。案イ採用なら本不明点は実装に影響しないため解消不要。
2. **T-099(c)調査5「Dock自己複製バグ」が現在の単一`MainDockingManager`構成でも再現するか**（§3.1）：当時の調査は4分割DockingManager時代のもの。案イでメニューにDock項目を含める場合、実装前に再検証が必要。

---

## 7. 検証計画（案イ採用を前提とした暫定案、他案採用時は別途調整）
...
3. §3裁定（案イ採用時）：Float発動メニュー項目の実装、`Tag`ベースの対象取得（`x:Name`参照でないこと、T-099(c)の教訓）。Dock項目を含める場合はT-099(c)調査5の再検証結果を踏まえた対処が組み込まれていること。
4. §4のAutoHide代替UI追加要否について、殿裁定（T121裁2）どおりに実装（または見送り）されていること。
...
2. **フロート時の見た目**：`PlacementToolBar`をタブドラッグでアプリウィンドウ外まで運びフロート化し、フロートウィンドウの見た目（帯の有無、案イなら消えたままのはず）を観察・記録（§6不明点1の実地裏取りを兼ねる）。
3. **フロート化・ドッキング操作の一巡**：§3裁定の代替メニュー（またはタブドラッグ）でFloat→Dock操作が正常に成立すること。Dock項目を含めた場合はタブ自己複製バグが再発しないこと（3〜4回反復）。
4. **タブドラッグでのフロート化**：物理マウスでアプリ内→アプリ外へドラッグしフロート化が成立すること（既に決着済みだが本改修後の回帰確認）。
5. **AutoHide操作**：T121裁2で「追加する」と裁定された場合のみ、表示メニュー経由でのAutoHide発動・復帰を確認。
6. **MainToolBar（CanFloat=False）のドラッグ無害性**：基本機能タブをアプリ外へドラッグしても何も起きない（フロート化しない）ことを確認（回帰確認）。
7. **Ctrl+Alt+Rリセット**：既定レイアウトへ戻った後も、両タブのドッキング時帯消失が維持されること。
8. **Tabキー巡回**：破綻しないこと。

### 7.3 実装規模の目安（家老の采配材料、案イ採用時）

- XAML: `TitleBarHiddenAnchorableControlStyle`へDataTrigger2本追加（約8行）、表示メニューへFloat項目1〜2個（約5行）。T121裁2=追加の場合はAutoHideメニュー項目2個+約4行。
- C#: Float/Dockハンドラ（約15行、AutoHide代替UIの`AutoHideMenuItem_Click`と同型）。T121裁2=追加の場合のみ`AutoHideSubmenu_SubmenuOpened`へ2行。
...
1. T121裁1（フロート時の扱い方針）・T121裁2（AutoHide代替UI追加要否）の殿裁定。
2. 案イ採用の場合、実装前にT-099(c)調査5「Dock自己複製バグ」の現構成での再現有無を軽く確認（Dock項目を含める場合のみ必須、Float発動のみに絞るなら不要）。
3. 侍実装（帯全体Collapseを1コミット、代替メニュー実装を別コミットに分けることを推奨）。
4. 隠密静的レビュー（§7.1）→忍者実機確認（§7.2）。
