...
| 図形 | 円2個＋横線1本(y=-0.1875)＋×印(横線と中心を揃え**横線をまたぐ**) | 円2個＋横線1本(y=+0.1875)＋×印(同形、横線をまたぐ) |

**通常の接点記号に、横線位置(aは上・bは下)と、その横線をまたぐ×印を足したもの**
（【2026-08-06訂正】既存調査書§2の初版記述「×印が横線の上/下」は誤り、正しくは横線自体の
y位置がa/bで異なり×印はどちらも横線をまたぐ同形。侍の実装時実測で判明、既存調査書側は
訂正済み）——電気的な振る舞い（role）は通常のa/b接点と同一で、図面上の記号だけが
サーマルリレー用に異なる。
