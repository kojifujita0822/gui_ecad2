...
## 0. 要点：機器表・BOMのPDF出力は「コードは存在するが到達不能」

**`DiagramRenderer.RenderBomPage`（786-804行、機器名/種別/型式/メーカー/数量の5列描画）・
`RenderCrossRefPage`は、App層から一切呼び出されていない**（`RenderBomPage`の呼び出し元は
`src`配下に存在せず、`MainWindow.xaml:118`「PDF出力」メニュー項目には`Click`/`Command`属性が
欠落）。Core層は完成済みだが、App層のPDFエクスポートオーケストレーションが未実装のため、
**現状のアプリではBOM表のPDF出力は事実上使えない**（T-060、隠密2の先行調査`docs/ecad2-t060-pdf-ui-wiring-survey-onmitsu2.md`と結論一致）。

**画面の機器表（`DeviceTableGrid`）とPDF側のBOM表は、コード上は同一ソートキー
（`Name`昇順・大小無視）・同一の種別表示文言（`DeviceClassLabel`）で設計されているが、後者は
現状到達しないコードパスである**点を混同しないよう注意。
...
新設）、**日本語ラベルはPDF出力の既存表記と統一する**（前述0節の「表記は統一されているが到達
不能」という状況の根拠）。

### T-066「BOM編集」（Approved、未着手）

`Device.Model/Maker/Quantity`はCore層完備・PDF出力のBOM表にも既に使用中だが、機器表DataGridが
`IsReadOnly="True"`で画面から編集不能。着手前調査は別途完了済み（`docs/ecad2-t065-t066-pre-investigation-onmitsu.md`）。編集可能にする列の範囲は着手時に殿確認【MUST】、未確定。
...
- `Device.Model`/`Maker`/`Quantity`への値セット手段が現状皆無（T-066未着手のため）。
- `DeviceClass`が登録時一度だけ決定され後から再計算されない設計の妥当性（要素種別変更時に追従
  すべきかどうかは未検討、T-066のような編集機能実装時に論点となりうる）。
