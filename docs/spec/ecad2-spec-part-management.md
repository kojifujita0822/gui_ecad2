...
- サムネイル表示：当時の固定5種+自作パーツ全件に24px角のサムネイル表示を確認（ただし部品選択
  リストの固定種がORa/ORbでなく「セレクトSW」という前提食い違いを検出した経緯もある、P-010起票→
  T-037で解消済み）。**（2026-07-21更新、T-071反映）T-071（2026-07-11、T-075完了直後）で経路B
  部品10種（押釦NO/NC・ランプ・モーター・タイマ接点NO/NC・タイマ瞬時接点NO/NC・サーマル・
  非常停止）が追加され、現在の組込みパーツは計15種**（`src/Ecad2.Core/Persistence/
  BasicPartTemplates.cs`）。旧版「固定5種」は現状と一致しない。
