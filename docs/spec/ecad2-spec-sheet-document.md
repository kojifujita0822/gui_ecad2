...
- `AddCommand`（`SheetNavigationViewModel.cs:90-142`）：名前が空なら`シート{連番}`へ自動フォール
  ...
  `UndoManager.RecordSnapshot`でスナップショット記録。**（2026-07-21更新、T-098反映）PageNumberは
  既存シートの最大PageNumber+1を採番する**（`DetermineNextPageNumber`、シート0枚なら1）。旧実装
  （`Sheets.Count+1`固定）は削除で欠番が生じた状態から追加すると既存PageNumberと重複するバグが
  あり、T-098（2026-07-21）で解消された。
...
- **（2026-07-21追記、T-084反映）削除でPageNumberに欠番が生じる場合は警告メッセージを表示する**：
  削除対象より大きいPageNumberを持つシートが1件でも残っていれば「シート削除によりページ番号に
  欠番が生じました。」とステータスバーに表示する（実際の再採番は行わず現状維持、P-066）。DRC
  結果がある状態での削除時は「DRC結果が削除されました。DRC再実行してください。」も連結して
  表示される。
