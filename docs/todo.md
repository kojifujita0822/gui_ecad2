...
GuiEcadより実装コスト低）。
**【殿確認待ち・保留中】UI/UX論点4点**（隠密2が選択肢化済み、殿帰参後に諮る）：
①挿入トリガーの配置場所（メニュー/ツールバー/両方）②配置位置決定方式（マウスホバー基準 vs
ecad2のSelectedCell基準——キーボードファースト方針との整合が論点）③リサイズ操作方式（数値入力のみ
踏襲 vs ドラッグハンドル新設）④対応ファイル形式（bmp/pngのみ vs jpg等追加）。
