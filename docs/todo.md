...
| 1 | **T-062** | **.NET 10移行**（ブランチ`t062-net10-migration`） | **SDK導入済み。T-051決着後・新機能実装前に実施** |
| 1.5 | **T-072** | **v0.3仮リリース** | T-062マージ完了が着手条件（殿裁定=マージできたらv0.3、T-057手順踏襲） |
| 2 | T-063/T-065/T-066/**T-071** | メニュー露出2件・文書情報・BOM編集・**経路B部品追加** | 棚卸し・部品差分起票（殿裁定2026-07-11）。T-062後の肩慣らし |
...
### T-072 v0.3仮リリースビルドの作成 — Approved（gated、殿直接指示2026-07-11）
...
次＝環境安定後に忍者が主要操作フローから実機回帰を再開→家老マージ→T-072（v0.3）。
**P-056（新規、家老所見）**：本日の中断契機＝殿PCでキーボード入力が完全に不通となるフリーズが
発生し、殿がスタートメニュー経由で手動再起動（マウスは生存・BSOD/BugCheckのシステムログ記録は
無し）。直前の忍者セッションの最終操作が、Ecad2.Appの「開く」ダイアログへ`SendKeys::SendWait`で
ファイルパス直接入力＋Enterを送る手順であり、その完了ログが残らぬまま途切れていた。技術的仮説
（未確定・再現未実施）として、.NET `SendKeys`のジャーナルフック方式がシステム全体のキーボード
入力を巻き込んで凍結させた可能性を家老が指摘。`ecad2-ui-automation`スキルは元々SendKeysを
「フォーカス強奪注意」としているが、システム規模の入力凍結は初の疑い事例。要否判断（再現検証・
SendKeysをクリップボード経由等へ置換する対策の要否）は殿裁定待ち、pending記録のみで着手せず。
