...
**進め方**：
1. **実機での症状確認**（忍者）——とりわけP-101は隠密所見が推測に留まる。**「本当に変更手段が無いのか」
   「変えたとき何が起きぬのか」を実測で確定させてから設計に入る**
2. **GuiEcad原本の種別選択UI・向き設定UIの形式調査**（隠密）——踏襲元がどう見せておるか
3. **UI形式は必ず殿へ選択肢を提示して裁可を得る**【MUST】（プロパティパネルへ追加か／専用ダイアログか／
   コンテキストメニューか。`karo.md`「UI/UX・使用感に関わる分岐は必ず殿へ」）
4. 侍の実装 → 検証パイプライン既定順
