...
1. **α**＝境界ガード（`ValidatePlacement`）適用箇所の網羅率（着手前＝何経路中いくつか、を先に測る）
   - **侍の着手前実測（2026-07-27、T-126へ振り替える直前まで測った分）＝7経路中1経路**。
     シートへの実体追加は7種9箇所（Elements/WireBreaks/ConnectionDots/Images/Frames/Connectors/
     FreeLines）、うち`ValidatePlacement`を通すのは要素配置1経路のみ（呼び出し3箇所＝配置1・移動2）
2. **β**＝ヒットテスト優先順位ロジックの実装箇所数（現状4系統 → 1系統）
   - **【要突合】数え方が三者で食い違っておる**——本欄「4系統」／隠密の調査書δ「左右2メソッド」／
     侍の実測「3系統（`MainWindow.xaml.cs`の左クリック押下1433〜・左クリック選択1918〜・
     右クリック2004〜）」。**侍自身が「己の数え方が誤っている見込みもあり断定せぬ」と留保**して
     おる。T-125再開時、まず数え方の定義を揃えてから着手前の値を確定すること
