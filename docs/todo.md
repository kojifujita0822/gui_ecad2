...
### T-055 行数拡張のGuiEcad方式踏襲＋母線番号入力の同仕様化 — Approved（gated、殿直接指示2026-07-10）
...
調査書を受けて設計・増分計画を殿へ提示してから侍実装に入る。UI/UXの分岐（GuiEcad仕様から
逸脱・取捨選択が必要な箇所）は必ず殿確認【MUST】。
...
### T-050 TraceLogの全角ラテン文字正規化統一 — In-progress（修正往復2周目、gated、殿裁定2026-07-10）

**往復経過**：初回実装`3190226`→隠密レビューで指摘2件（サロゲート例外・SelectedSheet old==new実バグ）
＋P-044同梱→1周目修正`e2f44d7`（経路X採用・RED証明2系統成立）→隠密事後レビューで**新規バグ発見＝
2周目へ**（AddCommand/DeleteCommandがCurrentSheetIndexセッタ経由で二重発火＋old値不整合、
ResetSheetsも呼び出し元の先行代入で誤old値。3独立エージェント一致CONFIRMED相当、実害はTraceLog
診断ログ限定。詳細=`docs/ecad2-t050-fix-review-onmitsu.md`）。制度どおり隠密テスト設計→侍修正で
2周目実施中。軽微2件（reuse・重複判定）は自然解消のみ許容、残れば経過観察。
**クローズ時は往復2周以上の制度適用＝テストコード静的レビュー+Stryker手動棚卸し（隠密）。**
