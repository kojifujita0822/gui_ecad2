...
**【高さ（ツールバーを覆う）】＝殿裁定2026-07-27＝「対処を検討する」**——**忍者の実測で依然覆って
おることを確認済み**（フライアウトY=117・高さ686、ツールバーは概ねY=95〜135）。**AvalonDock標準の
振る舞い**ゆえ、対処するならT-110増分1の設計判断に触れる（規模大）。
**隠密へ対処案の洗い出しを采配済み（2026-07-27）**＝(1)選択肢の列挙 (2)各案の影響範囲と規模
(3)T-110増分1の設計判断を巻き戻すことになるか否か。**規模が判明した時点で家老が殿へ改めて諮る**。
**一時計装（診断ログ、P-141）は本判断が済むまで残置**。
