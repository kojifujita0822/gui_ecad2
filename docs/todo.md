...
**【残・殿の判断待ち】高さ（ツールバーを覆う）**——**忍者の実測で依然覆っておることを確認済み**
（フライアウトY=117・高さ686、ツールバーは概ねY=95〜135）。**AvalonDock標準の振る舞い**ゆえ、
対処するならT-110増分1の設計判断に触れる（規模大）。**一時計装（診断ログ）は本判断が済むまで残置**。

**【残・殿の判断待ち】他パネルも同じ穴**——機器表・プロパティ・出力も保存XMLに`AutoHideWidth`を
持たぬ（`AutoHideMinWidth="100"`のみ）。**忍者の実測＝機器表・プロパティは左サイド上部、出力は
左サイド下部にタブが出て、出力パネルは下端から全幅で開く。細さの実害は見て取れず、シートパネル
ほど目立たぬ**（縦置き／横置きの差か、忍者は判じ兼ねると留保）。
