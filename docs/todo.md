...
**想定される対処案**（**実測前の当たりゆえ確定にあらず**。網羅は調査者の判断で補われたい）
- (a) 起動時ウィンドウ幅の既定値を広げる
- (b) 描画エリアが収まるよう起動時に自動算出
- (c) 最大化起動
- (d) 前回のウィンドウサイズを記憶

**留意＝(d)はP-052と直結する**。`proposed.md` P-052＝「アプリ全体のユーザー設定永続化機構が皆無」
（ウィンドウサイズ・パネル幅・表示設定をアプリ再起動間で記憶する仕組みが無い）。(d)を採ると規模が
基盤整備まで跳ね上がり、単独では収まらぬ。実測結果次第ではP-052の扱いも併せて殿へ諮ること。
