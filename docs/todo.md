...
| 順 | task | 内容 | 状態・根拠 |
|---|---|---|---|
| 0 | T-052 / T-059 / T-053 / T-054 | DRC警告・出力パネル高さ・種別列日本語化・選択部品表示 | 実装・レビュー完了（T-054レビューのみ残）、忍者実機確認待ち |
| 0 | T-055増分3 | 要素ごと削除 | 往復1周目クリーン、忍者実機で完全クローズ |
| 0 | T-051 | Undo/Redo基盤MVP | レビューで重大4件（データ破損2件含む）→テスト設計→往復1周目修正へ |
| 0 | T-056 | グリッド線表示切替 | 裁定4点確定済み、侍実装中 |
| 1 | **T-062** | **.NET 10移行** | **殿直接指示（早期移行）。進行中の決着後・新機能実装前に実施** |
| 2 | T-060 | PDF出力のUI結線 | 調査完了。着手時に論点4点の殿裁定 |
| 3 | T-061 | テストモードのUI結線 | 調査完了。着手時に論点6点の殿裁定 |
| 4 | T-058 | パネルのドック化・フロート配置 | AvalonDock導入裁定済み。T-062移行後にPoC（net10明示対応で不安消滅） |

- 調査はすべて完了（T-060/T-061/T-058/T-056）、以後は実装と裁定のみ
...
忍者実機確認待ち（チェック⇔描画連動・Ctrl+G実キー・非表示時に要素/母線残存の目視）。
...
隠密実測で全合格確認。次＝忍者実機確認（T-052/T-059と合流、増分3クローズの最終関門）。
...
### T-054 部品選択リストの選択中部品を配置バー内に表示 — In-progress（実装完了・隠密レビュー待ち、殿裁定2026-07-10）
...
全合格確認。忍者実機確認待ち（見た目・選び直し追従）。

### T-053 機器表「種別」列の日本語表示化 — In-progress（実装完了・隠密レビュー待ち、殿裁定2026-07-10）
...
テスト済み）とも問題なし、隠密実測で全合格確認。忍者実機確認待ち（表示崩れ・列幅）。
