...
- **論点2（描画消失）＝環境GPU HW描画不全で確定**。`DisableHWAcceleration=1`環境下でタブ切替往復
  (基本機能→配置ツール→基本機能)を实施したところ描画消失は一度も再現せず。実装回帰ではない。
...
が、增分1完了時点で申し送り済みの**「ドロップ枠判定範囲の全域拡大（仮実装として許容）」**により、
...
（增分1由来、增分2以降へ申し送り済み）の裏付けと確定**。同時に**T-103独自ドロップ枠自体の直接確認
も完了**（アプリ内全域に表示される挙動を実機で確認、範囲の精緻化は今回のスコープ外＝仮実装のまま
許容を継続）。

**增分2（回帰総点検）これにて完了**。所見A（範囲外・P-119）・所見B（バグでなし・決着）・所見C
（環境要因+既知仮実装の裏付け・決着）、いずれも实装回帰なしと確定。T-103含む対象7件、回帰なし。
