...
**規模＝小**（既存ダイアログへ項目を並べるだけ）。ただし**項目の並び順・入力形式（スピナーか
コンボボックスか等）はUI/UX分岐ゆえ殿へ確認する**【MUST】。

**留意**：**列数を減らす方向の変更で既存要素がはみ出す**場合の扱い（クランプするか・警告するか・
拒むか）は仕様判断を伴う。侍は実装前にこの点を洗い出し、殿の裁可を仰ぐこと。
