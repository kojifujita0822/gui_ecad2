...
### 実装順ロードマップ（2026-07-14家老全面更新、T-061/T-070完全決着・T-087/T-088着手を反映）

進行中の決着 → 小粒タスクの消化 → 大物の順（.NET 10移行=T-062は完了済み）。
...
| 順 | task | 内容 | 状態・根拠 |
|---|---|---|---|
| 1 | T-087 | ショートカットキー追加（部品パネルF11・テストモードCtrl+T） | 往復3周目、隠密レビュー中 |
...
| 4 | T-058/T-083 | パネルのドック化・フロート配置+ダークモード搭載（連動） | AvalonDock導入裁定済み、導入前PoCから着手 |
