...
**専用ブランチで作業する【MUST】**（mainを直接汚さぬリスク回避、殿指示2026-07-08＝T-045当時に
確立。ただしブランチ運用の可否は調査結果を見て改めて殿へ確認する）。
...
1. **α**＝境界ガード（`ValidatePlacement`）適用箇所の網羅率（着手前＝何経路中いくつか、を先に測る）
2. **β**＝ヒットテスト優先順位ロジックの実装箇所数（現状4系統 → 1系統）
3. **γ/δ**＝`MainWindowViewModel.cs`のメンバー数（現状283）、`MainWindow.xaml.cs`の行数
   （現状4,132）・ハンドラ数（現状143）・`case`文数（現状49）
...
| 順 | task | 内容 | 状態・根拠 |
|---|---|---|---|
| 1 | T-068 | 自作パーツ管理・編集UI | In-progress（増分0〜2完了、増分3を3段分割して進行中） |
