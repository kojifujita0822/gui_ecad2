...
| 0 | T-051 | Undo/Redo基盤MVP | 往復1周目修正中（重大4件、侍）→隠密再レビュー→忍者実機 |
| 1 | **T-062** | **.NET 10移行** | **殿直接指示（早期移行）。T-051決着後・新機能実装前に実施** |
| 2 | T-060 | PDF出力のUI結線 | 調査完了。着手時に論点4点の殿裁定 |
| 3 | T-061 | テストモードのUI結線 | 調査完了。着手時に論点6点の殿裁定 |
| 4 | T-058 | パネルのドック化・フロート配置 | AvalonDock導入裁定済み。T-062移行後にPoC（net10/5.0とも追い風） |
