...
| 2 | T-063/T-065/T-066/**T-071**/**T-074** | メニュー露出2件・文書情報・BOM編集・**経路B部品追加**・**バージョン情報表示** | 棚卸し・部品差分起票（殿裁定2026-07-11）。T-062後の肩慣らし |
...
### T-074 「バージョン情報」ダイアログへのバージョン番号表示 — Approved（gated、殿直接指示2026-07-11）
...
内容：メニュー項目へCommand結線＋ダイアログ（シンプルな形式で可）でアプリ名・バージョン番号を表示。
バージョン番号は`csproj`の`<Version>`と連動する既存の取得手段（`Assembly.GetName().Version`等）を
用いる（ハードコード禁止、次回以降のリリースでVersionを上げる度に自動反映されること）。
規模小。ロードマップ位置=T-062後の肩慣らし群（T-063/065/066/071）と同グループ。
