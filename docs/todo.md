...
### T-097 ラベル高さオフセット（LabelDy）入力UI新設＋コイル機器名の中心配置検証 — Approved（gated、殿直接指示2026-07-15）

**起票=殿直接指示2026-07-15**。`ParamKeys.LabelDy`（ラベル密集時の重なり回避用個別微調整）を
設定するUIがApp層に皆無と判明（隠密調査、`docs/ecad2-paramkeys-unwired-survey-onmitsu.md`、
PR-16「1パラメータのみの書き込み経路欠落」6例目）。
**UI仕様（殿直接指示2026-07-15）＝現在の高さを0として+-で上下させる**：種別既定の表示位置
（`ElementCatalog.DefaultLabelDy`）を基準0とし、そこからの相対オフセットとして入力する方式。
**併せて確認事項（殿直接指示2026-07-15）＝コイルの機器名は丸の中心に記入したい**：
`DefaultLabelDy(ElementKind.Coil) = -5.5`（`ElementCatalog.cs:49`、コメント「コイルの丸の中
あたりに表示」）が実際に丸の中心にあるか要検証。既に「中あたり」を意図した値が設定されている
ものの、殿の指摘は現状ズレている可能性を示唆——**着手前に実機で現状の表示位置を確認し、中心
からズレていれば既定値-5.5を補正してから、その補正後の値をLabelDy=0の基準とする**。
**DoD**：(1) コイル機器名の中心配置検証・必要なら既定値補正 (2) プロパティパネルへLabelDy
入力欄新設（相対オフセット方式） (3) 入力値を`Element.Params[LabelDy]`へ反映 (4) 既存の描画
ロジック（`DrawElementLabel`）は変更不要（器は完備） (5) 保存/読込で値保持 (6) 単体テスト+
build/test全合格。
**優先順位（殿裁定2026-07-15）**：Setpoint・LampColorと並行して順次進める。
**着手前調査完了（2026-07-15、隠密、`docs/ecad2-t096-t097-precheck-onmitsu.md`）＝理論計算**：
丸の中心=`YRow(row)`・半径3.78mm（`SymbolGlyphs.Coil`）。現状dy=-5.5でのラベル位置
yn=`YRow(row)+1.0`（VAlign.Bottom基準）。**テキスト矩形中心ベースのズレはごく僅か（0.2mm）**
——フォントのアセント/ディセンダー非対称性を考慮すると視覚的重心はさらに0.3〜0.8mm程度上へ
ズレる可能性があり（実測未了、静的解析の限界）、合算で中心から上へ0.5〜1.0mm程度ズレている
と推測。**進め方（家老裁量2026-07-15）＝まず現状-5.5を新基準0点として実装し、実機実測で
ズレを確認してから必要なら基準点（`DefaultLabelDy`）を補正する**（方向性の目安：-5.5→
-5.0〜-4.5程度、実測次第）。
副次気づき（スコープ外）：`ElementKind.Timer`（タイマコイル本体）は`DefaultLabelDy`のswitch
にcase無く既定0.0、Coilと異なり中心補正が入っていない（今回対象外、参考情報）。
...
| 順 | task | 内容 | 状態・根拠 |
|---|---|---|---|
| 1' | T-083 | ダークモード搭載（AvalonDock連動） | **T-058完全Done（2026-07-15）に伴い着手可能**。VS2013テーマ連動の追加PoC検証から |
| 1'' | T-096/T-097/T-085 | タイマー設定時間/LabelDy/ランプ色の入力UI新設 | **殿裁定（2026-07-15）＝順次進める最優先枠**。T-085は実装完了・実機確認待ち、T-096/T-097は着手前調査・殿裁定済み、詳細は各節参照 |
