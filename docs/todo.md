...
增分1（骨格統合の本実装）着手可能な状態。

**增分1（骨格統合の本実装）完了（2026-07-22）**：侍実装（`a78b802`）→隠密フル観点静的レビュー
（着手前チェック`docs/ecad2-t110-increment1-pretask-check-onmitsu.md`との1対1突き合わせ、指1
DockAsDocument封止を差し戻し`e1c8f73`）→忍者实機確認14項目（`docs/ecad2-t110-increment1-verification-ninja.md`）
の順で進行。(4)アクティブ色一元化（T-110発端・最重要）は本実装でも明確にOK。

**検出・解消した重大所見3件**：
1. **(2)起動時選択タブNG**：`SelectedContentIndex`が実機で反映されない既知事象（增分0由来）
...
2. **(9)レイアウト保存が無反応**：バグ対応のWチェック（侍調査+隠密独立調査を並行）で追跡。
   ...
   キーバインディング判定に到達すらしていないと確定）で裏付け、**实装バグでなく検証手法
...
提案（設計書§7）に従い、增分3の侍実装は增分2（回帰総点検）完了後に着手する。

**增分2（回帰総点検）一通り完了（2026-07-22、忍者）**：対象7件中T-100/T-101/T-104/T-106/T-107/T-108は
回帰なし確認。**T-103のみ独自ドロップ枠自体の直接確認が未完**（後述所見Cによりドラッグ操作を保留）。

**增分2检証中に検出した重大所見3件（調査中）**：
1. **所見A**：要素配置ツール選択中(`IsMainContentEnabled=False`連動)、メニューバー全体がLight/Dark
   問わず常にライト固定色になる。增分1範囲内(単一MainDockingManager化の影響)か既存見落とし
...
3. **所見C**：配置ツールタブ(T-104タブストリップ)をドラッグするとフロート化せずタブ切替が発生、
   ...
   ドラッグ特有の限界か实装回帰か忍者からは断定不可、隠密が調査中。

3件とも增分3・T-110完了の前提条件（範囲内欠陥として解消するか経過観察とするかの判断含む）。

**所見A・B・C 調査状況更新（2026-07-22）**：
- **所見A**：隠密調査完了（`docs/ecad2-t110-increment2-findings-ab-investigation-onmitsu.md`）＝增分1
  範囲外（既存事象）がほぼ確実。T-110の修正対象から外し`docs/proposed.md`P-119へ記録、殿裁定待ち。
- **所見B**：隠密の独立診断も静的機序を発見できず（両テーマの色定義は完全同値と確認）、動的実測が
  必要と確定。侍が診断ログを仕込み（一時計装、コミット未実施）、忍者へ实機测定（画素採取+ログ確認）
  を依頼中。
- **所見C**：隠密調査完了（`docs/ecad2-t110-increment2-finding-c-investigation-onmitsu.md`）＝**实装
  回帰の証拠なし**。論点1（タブドラッグでフロート化せず選択切替）はUIA合成ドラッグの限界+AvalonDock
  正規のタブセマンティクスで完全説明可能、論点2（描画消失）は增分0で確定済みの環境GPU HW描画不全の
...
**起票=殿直接指示2026-07-21**「内部処理は英語で問題ないが表示は日本語にしたい」（T-077增分6の
...
**增分6完了（2026-07-21、隠密）**：`docs/usage/ecad2-usage-statusbar.md`に「ツール表示の
