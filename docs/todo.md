...
**進め方**：
1. **実機での症状確認**（忍者、未了）——「本当に変更手段が無いのか」を実測で確定させる。
   **隠密調査で原本側は確定したが、ecad2側の現況はなお推測**（`memory:
   feedback_preliminary_survey_not_final`）
2. 侍の実装（P-100＝プロパティパネルへComboBox追加／P-101＝配置前選択メニューの新設）
3. 検証パイプライン既定順（隠密静的レビュー → 忍者実機確認）
...
**多重開始ガードの新設（侍の案B）＝保留**。**実害の実在が未確定のまま防御を積み増さぬ**
（`memory: feedback_verify_each_fix_before_stacking_next`）。上記1の隠密調査で実在が確定すれば、
規模を添えて殿へ諮る。
