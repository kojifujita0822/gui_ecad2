...
**【殿確認待ち】機器名変更のフォーカス誘導の具体的挙動**はUI/UX分岐のため殿帰参後に諮る、それ以外の
即着手可能部分は家老裁量で侍へ采配準備中（T-071完了後）。
...
**【殿確認待ち】UI/UX論点3点**（選択肢化済み、殿帰参後に諮る）：①UI形式・配置場所（上部バー/
下部パネル/ダイアログ/両方）②検索対象範囲（完全一致のみ/部分一致も/コメント等も含む——ただし
コメント編集UI自体がT-069調査で皆無と判明済みにつき対象拡大は非推奨）③置換1件の挙動（GuiEcad
同様1要素のみ vs 常に全置換扱いに統一）。
...
**【殿確認待ち】UI/UX論点4点**（選択肢化済み、殿帰参後に諮る）：①作成操作方法（マウスドラッグ/
キーボードステップ/両方）②配置単位（グリッドセル/mm自由、P-050対処要否と連動）③ラベル編集UI
方式・トリガー④移動操作方式。
