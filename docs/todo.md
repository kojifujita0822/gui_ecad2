...
2周目実施（`1d6db37`、RED証明8件・268件全合格。バグ2は侍裁量で通知移動方式=隠密が等価性検分済み）。
**2周目の隠密再レビュー=クリーン・要修正なし**（家老指定5観点すべて妥当、
`docs/ecad2-t050-fix2-review-onmitsu.md`）。軽微2件（reuse・重複判定）は残置=経過観察扱い。
**【殿判断待ち】**テストコード静的レビューで新規指摘1件=DeleteCommand境界値テスト4件が削除後の
SelectedSheet実値を未検証（退行検知力の穴、1行アサート追加x4で安価に解消可）——**往復3周目として
補強するか経過観察とするかの裁定が必要**（往復2周超えはゲートの掟）。裁定後=忍者実機
（シート追加/削除/改名+DRCジャンプ回帰）→隠密Stryker棚卸し→クローズ。
