...
### T-078 todo.mdの軽量化（完了済みタスクの詳細をアーカイブへ移送） — Approved（gated、殿直接指示2026-07-12）

**起票=殿直接指示2026-07-12**「todo.mdも肥大化してきている。要約して縮小して」。現状の実態
（家老の事前検分）：`docs/todo.md`が958行まで肥大化。既存の軽量化運用（2026-07-09殿指示、冒頭の
運用ルール参照）どおり、「生きているタスク」節に完全Done/Rejected済みタスクの詳細な経緯（往復記録・
コミットハッシュ・隠密レビュー詳細等）が残ったまま蓄積されている。

内容：
1. 「生きているタスク」節で状態が**完全Done**（またはRejected）となっているタスクの詳細記述を洗い出す
   （T-051/T-052〜056/T-057/T-059/T-062/T-063/T-071/T-072/T-073/T-074/T-075等）
2. それぞれの詳細経緯を`docs/todo-archive.md`末尾へ移送（内容は変更せず移動のみ）
3. `docs/todo.md`側は該当タスクを`docs/todo-archive.md`の既存索引と同型の**1行索引**へ置き換える
   （例：`- [x] T-051 シート追加削除のUndo対応（往復3周、完全Done 2026-07-11）`）
4. 実装順ロードマップ表の完了済み行はそのまま（表自体は簡潔なので触らない）
5. 進行中（Approved/In-progress）のタスクは対象外、詳細のまま残す

侍へ委譲。スコープ境界：内容の要約・改変はしない、移動のみ。既存の索引スタイルに合わせること。

### T-077 「ヘルプ」→「使い方」画面の新設（docs/spec/転用） — Approved（gated、殿直接指示2026-07-11）
...
| 2 | T-065/T-066 | 文書情報・BOM編集 | 棚卸し起票、UI/UX裁定済み・侍実装待ち |
| 3 | T-060 | PDF出力のUI結線 | 調査完了。着手時に論点4点の殿裁定 |
| 4 | T-064/T-069 | 画像挿入・右クリック残り4系統（中粒） | **着手前調査完了**（隠密2）。UI/UX論点は殿確認待ち |
| 5 | T-061 | テストモードのUI結線 | 調査完了。着手時に論点6点の殿裁定 |
| 6 | T-067/T-070 | GroupFrame作成・検索置換（中〜大） | 両方**着手前調査完了**（隠密2） |
...
Maker/Quantity列の追加・編集は今回スコープ外。侍へ実装采配は次段取り。
...
**殿裁定（2026-07-12、プレビュー提示で選択）**：①出力範囲＝**全シート常に（GuiEcad踏襲）**
（現在シートのみの選択肢は設けない） ②プレビュー機能＝**今回実装**（GuiEcad同様「プレビュー→
確認→エクスポート」の2段階UI、WPF側は新規描画基盤設計が必要——規模大、実装コスト増を承知の上での
裁定） ③枠あり/枠なし切替＝**切替UIなし、常に枠あり（`EnableBorder`既定値true固定）**。
エラー表示は実装時に侍が既存パターンを確認して判断（UI/UX分岐ではなく技術確認事項）。
