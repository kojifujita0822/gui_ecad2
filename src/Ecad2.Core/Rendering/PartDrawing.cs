...
/// T-068増分3-b1（家老裁可5・2026-07-25）: 形状編集キャンバス（増分3-b2）が
/// 「選択中の1プリミティブだけ別色」「作図中ドラフトを破線で」といったプリミティブ単位の
/// 描き分けを要するため、<see cref="DrawPrimitive"/> を切り出して public 化した
/// （App 層から個別に呼べるようにする。従来の <see cref="Draw"/> の挙動は不変）。
/// </summary>
public static class PartDrawing
{
    public static void Draw(IRenderer r, DrawingTheme theme, PartDefinition part, double cell, StrokeStyle s)
    {
        foreach (var p in part.Primitives)
            DrawPrimitive(r, theme, p, cell, s);
    }

    /// <summary>プリミティブ1つを描く。<see cref="Draw"/> はこれを全プリミティブへ適用するだけで、
    /// 描画ロジックの実体はすべて本メソッドに集約されている。</summary>
    public static void DrawPrimitive(IRenderer r, DrawingTheme theme, PartPrimitive p, double cell, StrokeStyle s)
    {
        switch (p)
        {
            case PartLine l:
