...
/// 差し替えるのが、テンプレートを複製せずに射程を閉じ込められる道</b>である。
/// <b>「唯一の道」ではない</b>——理屈のうえでは <c>ControlTemplate</c> 自体を複製して
/// <c>TargetName</c> の参照先を差し替える道も在る。ただし複製は将来のテーマ変更で
/// 更新漏れを生むゆえ、<b>複製より保守コストの低い最善の道</b>という物言いが正確である
/// （隠密の指摘により、侍が当初「唯一の道」と書いたのを改めた）。
/// </para>
/// <para>
...
{
    /// <summary>
    /// 部品リストへ局所的に置いた選択背景の上書き（方向A＝ダーク側と同値）。
    /// <para>
    /// <b>【この定数は二用途で使い回しておる】</b>観点1（ライトの局所上書きが在ること）と
    /// 観点4（ダークの定義が据え置きであること）の両方が本定数を参照する——方向Aにより
    /// 両者が同じ値になったゆえ成り立つ形である。
    /// <b>ゆえにダーク側の選択背景を変える折は、本定数を分ける要がある</b>
    /// ——分けねば観点1と観点4のどちらかが意図せず壊れる。
    /// </para></summary>
